...
export async function startWorker() {
  ...
  agenda.define(
    ...
    async (job: optimizeMorphoYieldJobDef.JobType) => {
      // TODO: add job-aware logic such as cool-downs in case of repeated failures here

      try {
        await optimizeMorphoYieldJobDef.processJob(job);
      } catch (err) {
        ...
        if (
          ...
          consola.log(`Disabling schedule job due to fatal error: ${error.message}`);
          job.disable();
          await job.save();
          throw new Error(`Schedule job disabled due to fatal error: ${error.message}`);
        }
